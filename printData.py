...
def printSleepData(sleep_data):
    ...
    try:
        ...
        print('>>> Sleep details <<<') if summary_date == YESTERDAY else print(
            f'>>> Sleep details [{summary_date}] <<<')
        ...
        print("Sleep details not available.")
    print()
...
def printActivityData(activity_data):
    ...
    try:
        print(
            '>>> Activity details <<<'
        ) if activity_data['summary_date'] == datetime.today().strftime(
            '%Y-%m-%d') else print(
                f'>>> Activity details [{activity_data["summary_date"]}] <<<')
        try:
            print(f"- Activity score: {activity_data['score']}")
        except:
            print("- Activity score: Not available")
        ...
        print('- Rest mode is on.'
              ) if activity_data['rest_mode_state'] == 1 else print(
                  '- Rest mode is off.')
    except:
...
def printReadinessData(readiness_data):
    ...
    try:
        print(
            '>>> Readiness details <<<'
        ) if readiness_data['summary_date'] == YESTERDAY else print(
            f'>>> Readiness details [{readiness_data["summary_date"]}] <<<')
        print(f"- Readiness score: {readiness_data['score']}")
        try:
            print(
                f"- Activity balance: {readiness_data['score_activity_balance']}"
            )
        except:
            print("- Activity balance: Not available")
        ...
        print("Readiness details not available.")
    print()
